...
class UsersRoute implements Routes {
  ...
  private initializeRoutes() {
    ...
    this.router.put(`${this.path}/:id(\\d+)`, [authMiddleware, validationMiddleware(CreateUserDto, 'body')], this.usersController.updateUser);
    this.router.delete(`${this.path}/:id(\\d+)`, authMiddleware, this.usersController.deleteUser);
    this.router.get(`${this.path}/change-to-superuser/:id(\\d+)`, authMiddleware, this.usersController.changeToSuperUser);
  }
}
