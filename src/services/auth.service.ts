...
class AuthService {
  ...
  public async login(userData: CreateUserDto): Promise<{ cookie: string; tokenData: any }> {
    if (isEmpty(userData)) throw new HttpException(400, 'Empty content');
    ...
    if (!isPasswordMatching) throw new HttpException(409, 'User does not found');
  ...
  public async logout(userData: User): Promise<User> {
    ...
    if (!findUser) throw new HttpException(409, 'User does not found');
